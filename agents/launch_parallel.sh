...
# Each pair gets a requested GPU, ROS2 domain, episode slice, and log directory.
# Jetson systems expose one integrated CUDA device, shared by all worker pairs.
#
...
if [[ "$BASELINE" == "trihelper" ]]; then
  if [[ ! -f "${BASELINE_DIR}/TriHelper/RedNet/model/rednet_semmap_mp3d_40.pth" ]]; then
    echo "ERROR: TriHelper RedNet weights are missing; see docs/baselines.md." >&2
    exit 1
  fi
...
echo "   GPUs           : $GPU_LIST"
if [[ "$JETSON_MODEL" == *Jetson* ]]; then
  echo "   Jetson GPU     : device 0 shared by all workers (${JETSON_MODEL})"
fi
...
if [[ "$BASELINE" == "beliefmapnav" ]]; then
  mkdir -p "${BASELINE_DIR}/BeliefMapNav/data"
  for weight in mobile_sam.pt groundingdino_swint_ogc.pth yolov7-e6e.pt pointnav_weights.pth; do
    if [[ ! -s "${DATA_DIR}/${weight}" ]]; then
      echo "ERROR: beliefmapnav weight is missing: ${DATA_DIR}/${weight}" >&2
      exit 1
    fi
  done
  if [[ ! -s "${BASELINE_DIR}/BeliefMapNav/data/rednet_semmap_mp3d_40.pth" ]]; then
    echo "ERROR: BeliefMapNav RedNet weights are missing; see docs/baselines.md." >&2
    exit 1
  fi
...
_agent_extra_volumes() {
  case "$BASELINE" in
    ...
    beliefmapnav)
      ...
      echo "      - ${BASELINE_DIR}/BeliefMapNav/data:/opt/beliefmapnav_local_data:ro"
...
CYCLONEDDS_CONFIG="cyclonedds.xml"
if [[ "$BASELINE" == "vlfm" ]]; then
  # VLFM's env and agent share host networking. Loopback keeps DDS stable when
  # a physical interface (especially Wi-Fi) disconnects during evaluation.
  CYCLONEDDS_CONFIG="cyclonedds_loopback.xml"
fi
...
x-common: &common
  ...
  environment: &common-env
    ...
    # Legacy official checkpoints (YOLOv7/PointNav) contain serialized model classes.
    TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD: "1"
    RMW_IMPLEMENTATION: rmw_cyclonedds_cpp
    CYCLONEDDS_URI: file:///opt/rt_ovn/utils/${CYCLONEDDS_CONFIG}
    MODE: ${MODE}
    BENCHMARK: ${BENCHMARK}
    BASELINE: ${BASELINE}
    VLFM_MODE: ${MODE}
...
for (( i=0; i<NUM_WORKERS; i++ )); do
  ...
  env_${i}:
    ...
    # Jetson requires the NVIDIA runtime. Compose device reservations invoke
    # the hook directly and fail during OCI container initialization.
    runtime: nvidia
    environment:
      <<: *common-env
      NVIDIA_VISIBLE_DEVICES: "${GPU}"   # requested device; Jetson workers are normalized to GPU 0
...
EOF
    ...
    case "$BASELINE" in
      ...
      YOLOV7_PORT: \"${YOLO_PORT}\"
      POINTNAV_WEIGHTS: data/pointnav_weights.pth
      REDNET_WEIGHTS: /opt/beliefmapnav_local_data/rednet_semmap_mp3d_40.pth
      ...
      VLLM_PORT: \"${VLLM_PORT}\"
      RTNAV_LLM: \"${RTNAV_LLM}\"
      RTNAV_VLLM_MAX_NUM_SEQS: \"1\"
      RTNAV_VLLM_STARTUP_TIMEOUT: \"600\"
