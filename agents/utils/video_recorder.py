...
class _BaseRecorder:
    ...
    def __init__(self, video_dir: Optional[str], prefix: str, fps: int = 10):
        ...
        # ROS observation callbacks and the evaluation loop run on different
        # threads. ImageIO's writer must never be opened, written, or closed
    ...
    def save_episode(self, metrics: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        """
        with self._writer_lock:
            return self._save_episode_locked(metrics)

    def _save_episode_locked(
