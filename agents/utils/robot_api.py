...
class RobotAPI:
        ...
        for _ in range(int(10.0 / self._cmd_period)):
            ...
            if cur.step_id != prev_step:
                if abs(dist - prev_dist) < 0.001:
                    ...
                    stall_since = time.monotonic()
                prev_step, prev_dist = cur.step_id, dist
    ...
    def _run_turn(self, direction: int) -> Optional[ObservationMsg]:
        ...
        for _ in range(int(10.0 / self._cmd_period)):
            ...
            if abs(progress - prev_progress) < 0.001:
                ...
                stall_since = time.monotonic()
            prev_step, prev_progress = cur.step_id, progress
    ...
    def _run_look(self, direction: int) -> Optional[ObservationMsg]:
        ...
        for _ in range(int(10.0 / self._cmd_period)):
            ...
            time.sleep(self._cmd_period)
            cur = self.get_latest_observation()
            if cur is not None and cur.step_id != prev_step:
                latest = cur
    ...
    def _stop_and_wait(self, latest: ObservationMsg) -> ObservationMsg:
        settled = self._wait_for_fresh_obs(latest.step_id, timeout=1.0, keep_stopped=True)
    ...
    def _wait_for_fresh_obs(
        self, prev_step: int, timeout: float = 1.0, keep_stopped: bool = False
        ...
        while time.time() - t0 < timeout:
            ...
            if keep_stopped:
                self._publish_vel(0, 0, 0)
            obs = self.get_latest_observation()
            if obs is not None and obs.step_id != prev_step:
                return obs
