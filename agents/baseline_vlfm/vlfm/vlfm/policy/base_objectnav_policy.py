...
def resolve_detector_mode() -> str:
    mode = os.environ.get("VLFM_DETECTOR_MODE", "auto").strip().lower().replace("-", "_")
    if mode in ("", "auto", "benchmark"):
        benchmark = os.environ.get("BENCHMARK", os.environ.get("VLFM_CONFIG", "ovon")).strip().lower()
        return "yolo_gdino" if benchmark == "hm3d" else "owlv2"
    if mode in ("hm3d", "yolo", "yolo_gdino", "yolo_groundingdino", "groundingdino", "gdino"):
        return "yolo_gdino"
    if mode in ("ovon", "owl", "owlv2"):
        return "owlv2"
    raise ValueError(
        "VLFM_DETECTOR_MODE must be auto, owlv2, or yolo_gdino "
        f"(got {os.environ.get('VLFM_DETECTOR_MODE')!r})"
    )
...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__()
        self._detector_mode = resolve_detector_mode()
        detector_port = int(os.environ.get("GROUNDING_DINO_PORT", "12181"))
        if self._detector_mode == "yolo_gdino":
            self._object_detector = GroundingDINOClient(port=detector_port)
        else:
            # OWLv2 is an optional extension and requires Transformers newer
            # than upstream VLFM's pinned 4.26.  Import it only on that path so
            # the original YOLOv7 + GroundingDINO reproduction remains pinned.
            from vlfm.vlm.owlv2 import OWLv2Client

            self._object_detector = OWLv2Client(port=detector_port)
        ...
        self._load_yolo = self._detector_mode == "yolo_gdino"
        print(f"VLFM detector mode: {self._detector_mode}")
        if use_vqa:
    ...
    def set_target_object(self, target_object: str) -> None:
        if target_object == self._target_object:
            return
        self._target_object = target_object
        self._non_coco_caption = f"{target_object} ."
        self._pointnav_policy.reset()
        self._last_goal = np.zeros(2)
        self._called_stop = False
    ...
    def _pre_step(self, observations: "TensorDict", masks: Tensor) -> None:
        ...
        if not self._did_reset and masks[0] == 0:
            self._reset()
            # Support both integer objectgoal IDs (standard) and string names (OVON)
            goal = observations.get("objectgoal", None)
            if goal is None:
                goal = observations.get("objectgoal_text", "object")
            self._target_object = goal if isinstance(goal, str) else goal
        try:
    ...
    def _get_object_detections(self, img: np.ndarray) -> ObjectDetections:
        ...
        if has_non_coco:
            print(f"non coco caption: {self._non_coco_caption}")
        detections = (
