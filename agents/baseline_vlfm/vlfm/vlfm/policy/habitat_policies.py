...
try:
    ...
    from ovon.config import OVONPolicyConfig

    OVON_AVAILABLE = True
except ImportError:
    OVON_AVAILABLE = False

# Cache for reverse-looking up OVON goal category from CLIP embedding
_ovon_clip_cache = None
_ovon_clip_embeddings = None


def _get_ovon_goal_name_from_embedding(clip_embedding_tensor):
    """Reverse-lookup the OVON goal category name from its CLIP embedding."""
    global _ovon_clip_cache, _ovon_clip_embeddings
    import pickle
    import numpy as np

    if _ovon_clip_cache is None:
        try:
            cache_path = os.environ.get(
                "OVON_CLIP_CACHE",
                os.path.join(os.path.dirname(__file__), "..", "..", "data", "hm3d_ovon", "text_embeddings", "siglip.pkl"),
            )
            with open(cache_path, "rb") as f:
                _ovon_clip_cache = pickle.load(f)
            _ovon_clip_embeddings = {
                k: v / np.linalg.norm(v) for k, v in _ovon_clip_cache.items()
            }
        except Exception as e:
            print(f"Warning: Could not load OVON CLIP cache: {e}")
            return "object"

    embedding = clip_embedding_tensor.cpu().numpy().flatten()
    embedding = embedding / (np.linalg.norm(embedding) + 1e-8)

    best_cat = "object"
    best_sim = -1.0
    for cat, cached_emb in _ovon_clip_embeddings.items():
...
class HabitatMixin:
    ...
    def act(
        ...
        # Determine goal object name
        if OVON_AVAILABLE and ClipObjectGoalSensor.cls_uuid in obs_dict:
            self._is_ovon = True
            goal_name = _get_ovon_goal_name_from_embedding(
                observations[ClipObjectGoalSensor.cls_uuid][0]
            )
            obs_dict[ObjectGoalSensor.cls_uuid] = goal_name
        elif ObjectGoalSensor.cls_uuid in obs_dict:
            object_id = observations[ObjectGoalSensor.cls_uuid][0].item()
        ...
        # For OVON, set non_coco_caption to the target object name
        if self._is_ovon:
            goal_name = obs_dict.get(ObjectGoalSensor.cls_uuid, "object")
            if isinstance(goal_name, str):
                self._non_coco_caption = goal_name + " ."
    ...
    def _cache_observations(self: Union["HabitatMixin", BaseObjectNavPolicy], observations: TensorDict) -> None:
        ...
        # Extract OVON goal name if available
        if self._is_ovon and "ovon_goal_category" in observations:
            self._ovon_goal_name = observations["ovon_goal_category"]
...
@baseline_registry.register_policy
class HabitatITMPolicyOVON(HabitatMixin, ITMPolicyV2):
    """ITMPolicyV2 adapted for OVON evaluation."""
    pass


_PolicyConfigBase = OVONPolicyConfig if OVON_AVAILABLE else PolicyConfig


@dataclass
class VLFMPolicyConfig(VLFMConfig, _PolicyConfigBase):
    pass
