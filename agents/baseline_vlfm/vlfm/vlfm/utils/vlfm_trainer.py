...
def _write_vlfm_trace(path: str, episodes: list, current_episode: dict = None) -> None:
    if not path:
        return
...
class VLFMTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        num_total = 0
        hab_vis = HabitatVis()
        trace_path = os.environ.get("VLFM_TRACE_PATH", "").strip()
        trace_episodes = []
        trace_current = None
        if trace_path:
            ...
            _write_vlfm_trace(trace_path, trace_episodes, trace_current)
