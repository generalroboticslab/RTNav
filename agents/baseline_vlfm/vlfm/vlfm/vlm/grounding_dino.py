...
try:
    _GROUNDING_DINO_CONFIG = files("groundingdino").joinpath("config/GroundingDINO_SwinT_OGC.py")
    if not _GROUNDING_DINO_CONFIG.is_file():
        raise FileNotFoundError(_GROUNDING_DINO_CONFIG)
    GROUNDING_DINO_CONFIG = str(_GROUNDING_DINO_CONFIG)
except Exception:
...
class GroundingDINO:
    ...
    def predict(self, image: np.ndarray, caption: Optional[str] = None) -> ObjectDetections:
        ...
        caption_to_use = caption if caption is not None else self.caption
