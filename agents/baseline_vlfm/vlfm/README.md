...
## :arrow_forward: Evaluation on HM3D-OVON

VLFM can also be evaluated on the [HM3D-OVON](https://github.com/naoki-yokoyama/ovon) open-vocabulary ObjectNav benchmark.

### Prerequisites

1. Install the `ovon` package from the canonical `envs/ovon` source tree:
   ```bash
  pip install -e ../../envs/ovon
   ```
2. Download the OVON episode dataset and place it at `data/datasets/ovon/hm3d/v1/`.
3. Ensure VLM servers are running (`./scripts/launch_vlm_servers.sh`).

### Running evaluation

```bash
python -m vlfm.run_ovon
```

You can override config values as usual, for example:
```bash
python -m vlfm.run_ovon habitat_baselines.num_environments=4
```
