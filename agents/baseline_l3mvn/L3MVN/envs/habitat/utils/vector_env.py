...
WORKER_ERROR_COMMAND = "worker_error"
...
class VectorEnv:
    ...
    def __init__(
        ...
        self.observation_spaces = [
            self._read_worker_result(read_fn)
    ...
    def current_episodes(self):
        ...
        results = []
        for read_fn in self._connection_read_fns:
            results.append(self._read_worker_result(read_fn))
        self._is_waiting = False
        return results
    ...
    def reset_at(self, index_env: int):
        ...
        results = [self._read_worker_result(self._connection_read_fns[index_env])]
        self._is_waiting = False
        return results
    ...
    def call_at(
        ...
        result = self._read_worker_result(self._connection_read_fns[index])
        self._is_waiting = False
        return result
    ...
    def render(
        ...
        images = [self._read_worker_result(read_fn) for read_fn in self._connection_read_fns]
    ...
    def _read_worker_result(self, read_fn: Callable[[], Any]) -> Any:
        result = read_fn()
        if (
            isinstance(result, tuple)
            and len(result) == 2
            and isinstance(result[0], str)
            and result[0] == WORKER_ERROR_COMMAND
        ):
            raise WorkerEnvError(result[1])
        return result
