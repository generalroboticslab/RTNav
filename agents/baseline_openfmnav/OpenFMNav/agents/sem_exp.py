...
class Sem_Exp_Env_Agent(ObjectGoal_Env21):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        while True:
            try:
                self.GSAM = GSAM(self.object_category[:-1], text_threshold=text_threshold, use_ram=use_ram)
                break
            except Exception as ex:
                print(f"=====> [ERROR]: {ex}, sleep for 20s...")
                time.sleep(20)
                continue
    ...
    def plan_act_and_preprocess(self, planner_inputs):
        ...
        if planner_inputs["new_goal"]:
            ...
            if np.sum(goal == 1) == 1 and self.args.task_config == "tasks/objectnav_gibson.yaml":
                ...
                self.info["g_reward"] = self.get_fmm_distance(planner_inputs["map_target"], frontier_loc)
    ...
    def _visualize(self, inputs):
        ...
        no_cat_mask = sem_map == len(self.object_category) + 4
        ...
        if np.sum(goal) == 1:
            ...
            goal_fmb = skimage.draw.rectangle_perimeter(
                start=(int(f_pos[0][0])-int(local_w/args.boundary_coeff)//2, int(f_pos[0][1])-int(local_w/args.boundary_coeff)//2),
                end=(int(f_pos[0][0])+int(local_w/args.boundary_coeff)//2, int(f_pos[0][1])+int(local_w/args.boundary_coeff)//2),
            )
        ...
        sem_map_vis = cv2.resize(sem_map_vis, (980, 980),
                                 interpolation=cv2.INTER_NEAREST)
        self.vis_image[50:530, 15:655] = self.rgb_vis
        self.vis_image[550:550+480, 15:655] = self.semantics_vis
        self.vis_image[50:530+500, 670:1150+500] = sem_map_vis

        pos = (
            (start_x * 100. / args.map_resolution - gy1)
            * 980 / map_pred.shape[0],
            (map_pred.shape[1] - start_y * 100. / args.map_resolution + gx1)
            * 980 / map_pred.shape[1],
            np.deg2rad(-start_o)
        )
    ...
    def get_metrics(self):
        ...
        metrics['object_category'] = self.object_category
        return metrics
