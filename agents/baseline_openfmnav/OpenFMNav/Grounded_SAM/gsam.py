...
def convert_SAM(sam_semantic_pred, object_category, image_shape=None):
    '''
    ...
    try:
        ...
        if image_shape is None:
            image_shape = (480, 640)
        return np.zeros((image_shape[0], image_shape[1], len(object_category)))
...
class GSAM():
    ...
    def get_vis(self, rgb, predictions):
        ...
        if image is None:
            raise RuntimeError("Failed to render Grounded-SAM visualization")
        return cv2.resize(image, (640, 480), interpolation=cv2.INTER_NEAREST)
