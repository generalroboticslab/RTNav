...
def _require_habitat_api():
    if _HABITAT_IMPORT_ERROR is not None:
        raise ImportError(
            "OpenFMNav's original vector-env entry points require the legacy "
            "Habitat-Lab API. The ROS wrapper imports mapping/planning utilities "
            "without constructing these vector envs."
        ) from _HABITAT_IMPORT_ERROR


def make_env_fn(args, config_env, rank):
    _require_habitat_api()
