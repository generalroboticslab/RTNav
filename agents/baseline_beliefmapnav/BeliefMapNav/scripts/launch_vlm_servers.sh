...
tmux send-keys -t ${session_name}:0.0 "CUDA_VISIBLE_DEVICES=${GPUID} ${VLFM_PYTHON} -m vlfm.vlm.grounding_dino --port ${GROUNDING_DINO_PORT}" C-m
# tmux send-keys -t ${session_name}:0.1 "${VLFM_PYTHON} -m vlfm.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
# tmux send-keys -t ${session_name}:0.1 "${VLFM_PYTHON} -m vlfm.vlm.blip2 --port ${BLIP2_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "CUDA_VISIBLE_DEVICES=${GPUID} ${VLFM_PYTHON} -m vlfm.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "CUDA_VISIBLE_DEVICES=${GPUID} ${VLFM_PYTHON} -m vlfm.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
