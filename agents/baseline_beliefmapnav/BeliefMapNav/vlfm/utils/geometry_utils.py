...
def xyz_yaw_to_extrinsic(xyz: np.ndarray, yaw: float) -> np.ndarray:
    x, y, z = xyz
    R = np.array([[np.cos(yaw), -np.sin(yaw), 0],
            [np.sin(yaw), np.cos(yaw), 0],
            [0, 0, 1]])
    t = np.array([x,y,z])
    local_to_camera = np.array([[0, -1, 0],
            [0, 0, -1],
            [1, 0, 0]])
    R_e = local_to_camera @ R.T
    t_transformed = - R_e @ t
    extrinsic = np.vstack([np.hstack([R_e, t_transformed.reshape(-1, 1)]), np.array([0, 0, 0, 1])])
    return extrinsic


def xyz_yaw_pitch_roll_to_extrinsic(xyz: np.ndarray, yaw: float, pitch: float, roll: float) -> np.ndarray:
    """
    Build camera extrinsic (world -> camera) from position and yaw/pitch/roll (radians).

    Conventions
    ----------
    - Right-handed axes.
    - Rotations applied in order: yaw (Z) -> pitch (Y) -> roll (X).
    - `local_to_camera` matches your original code's axis remap.
    - Returns a 4x4 matrix [R_e | t_e; 0 0 0 1] that maps world points to camera.

    Args:
        xyz   : (3,) camera position in world (x, y, z)
        yaw   : rotation around world Z (rad)
        pitch : rotation around world Y (rad)
        roll  : rotation around world X (rad)

    Returns:
        extrinsic: (4,4) numpy array
    """
    x, y, z = xyz

    # Body/world rotation (camera pose in world)
    Rz = np.array([
    ...
    R_world = Rz @ Ry @ Rx  # yaw → pitch → roll
    t_world = np.array([x, y, z], dtype=float)

    # Same local_to_camera as your original
    local_to_camera = np.array([
        [0, -1,  0],
        [0,  0, -1],
        [1,  0,  0]
    ], dtype=float)
