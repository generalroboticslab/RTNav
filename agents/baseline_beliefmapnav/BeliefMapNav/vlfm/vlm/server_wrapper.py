...
def host_model(model: Any, name: str, port: int = 5000) -> None:
    ...
    """
    # rt_ovn author addition: the detectors run out of process, so determinism
    # has to be enabled here too or their outputs vary run to run.
    from vlfm.utils.determinism import enable_determinism

    if enable_determinism():
        print(f"[rt_ovn] determinism enabled for {name} server")
...
def _send_request(url: str, **kwargs: Any) -> dict:
    lockfiles_dir = os.environ.get("VLM_LOCKFILES_DIR", "lockfiles")
