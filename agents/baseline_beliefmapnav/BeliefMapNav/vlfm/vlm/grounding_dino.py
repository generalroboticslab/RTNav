...
# --- rt_ovn author addition (begin): honour the env vars scripts/launch_vlm_servers.sh
# already exports. Upstream hard-codes a repo-relative path into a cloned
# GroundingDINO/ tree, but readme_rtovn_authors.md step D installs GroundingDINO
# from pip, so that directory does not exist and the server dies at startup with
# FileNotFoundError. Defaults are unchanged when the vars are unset.
GROUNDING_DINO_CONFIG = os.environ.get(
    "GROUNDING_DINO_CONFIG",
    "GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py",
)
GROUNDING_DINO_WEIGHTS = os.environ.get(
...
class GroundingDINO:
    def __init__(
        ...
        print("config_path: ",config_path,"model_checkpoint_path: ",weights_path)
        self.model = load_model(model_config_path=config_path, model_checkpoint_path=weights_path).to(device)
        self.caption = caption
        self.box_threshold = box_threshold
        self.text_threshold = text_threshold
        print("self.box_threshold: ",self.box_threshold)
        print("self.text_threshold: ",self.text_threshold)
