        ...
        if update_obstacles:
            ...
            if agent_pitch_angle <= 0 and reach_stair == False: 
                ...
                if inverted_point_cloud_episodic_frame.shape[0] > 0:
                    ...
                    try:
                        ...
                        # rt_ovn author addition (fallback only): on large scenes
                        # some pixels fall outside the fixed-size map and upstream
                        # crashes; keep only in-bounds pixels and continue.
                        _h, _w = self._down_stair_map.shape[:2]
                        _ib = (
                            (below_ground_pixel_points[:, 0] >= 0) & (below_ground_pixel_points[:, 0] < _w)
                            & (below_ground_pixel_points[:, 1] >= 0) & (below_ground_pixel_points[:, 1] < _h)
                        )
                        _pts = below_ground_pixel_points[_ib]
                        self._down_stair_map[_pts[:, 1], _pts[:, 0]] = 1
            if search_stair_over == True: 
                ...
                try:
                    ...
                    # some pixels fall outside the fixed-size map and upstream
                    # crashes; keep only in-bounds pixels and continue.
                    _h, _w = self._map.shape[:2]
                    _ib = (
                        (pixel_points[:, 0] >= 0) & (pixel_points[:, 0] < _w)
                        & (pixel_points[:, 1] >= 0) & (pixel_points[:, 1] < _h)
                    )
                    _pts = pixel_points[_ib]
                    self._map[_pts[:, 1], _pts[:, 0]] = 1
