...
class DetectionEntity:
    ...
    confidence: float
    location: Tuple[float, float]  # (cx, cy)
...
class MultiCameraDetectionResult:
    ...
    timestamp: float
    robot_odom: Tuple[float, float, float]
...
class DetectedObjectInfo:
    """Target object detection for submission."""
    ...
    timestamp: float
    detection_pose: Optional[Tuple[float, float, float]] = None
...
class FrontierOutput:
    ...
    frontier_centroids_objects: List[List[str]]
