...
def project_mask_detection_to_3d(
    ...
    ry1i, rx1i = max(0, int(ry1) - 5), max(0, int(rx1) - 5)
    ry2i, rx2i = min(H_rgb, int(ry2) + 5), min(W_rgb, int(rx2) + 5)

    return ObjectDetection(
        label=det.name,
        bbox_2d=np.array(det.bbox),
        ...
        frame_idx=frame_idx,
        rgb_crop=rgb[ry1i:ry2i, rx1i:rx2i],
...
def _process_camera_cpp(
    ...
    resolution.  RGB crops stay full-resolution.
    """
    rgb = cam.rgb_image
    depth = cam.depth_image
    min_depth_m = float(getattr(cam, "min_depth_m", 0.0))
...
def process_all_cameras_parallel(
    ...
    frame_idx: int = 0,
) -> Tuple[List[ObjectDetection], int]:
    """Project all cameras' 2D detections to 3D via the C++ backend.
    Returns (valid_detections, count)."""
    ...
    for cam_name, cam in camera_results.items():
        ...
        results = _process_camera_cpp(cam, detections, frame_idx)
    ...
    return valid_results, len(valid_results)
