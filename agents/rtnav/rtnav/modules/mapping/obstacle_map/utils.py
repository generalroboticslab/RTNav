...
def find_navigable_goal(
    ...
    if obs_map.is_navigable_px(gx, gy):
        return tuple(goal_xy), goal_px

    radius = max(0, int(search_radius_m * obs_map.ppm))
    x0, x1 = max(0, gx - radius), min(obs_map.size, gx + radius + 1)
    y0, y1 = max(0, gy - radius), min(obs_map.size, gy + radius + 1)
    ys, xs = np.nonzero(obs_map.navigable[y0:y1, x0:x1])
    if len(xs) == 0:
        return None
