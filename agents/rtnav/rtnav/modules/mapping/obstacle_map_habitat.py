...
import cv2
import numpy as np
from frontier_exploration.frontier_detection import detect_frontiers, frontier_waypoints
...
def xyz_yaw_to_tf_matrix(xyz: np.ndarray, yaw: float) -> np.ndarray:
    """VLFM ``vlfm/utils/geometry_utils.py::xyz_yaw_to_tf_matrix`` (verbatim)."""
    x, y, z = xyz
    return np.array(
        [
...
class HabitatObstacleMap:
    ...
    def update_map(
        ...
        Args:
            ...
            tf_camera_to_episodic: 4×4 world-from-camera matrix. For
                ...
                (``xyz_yaw_to_tf_matrix([gps[0], -gps[1], h], compass)``).
    ...
    def _get_frontiers(self) -> np.ndarray:
        ...
        boundaries_px = detect_frontiers(
            nav_u8,
            explored_area,
            self._area_thresh_in_pixels,
        )
        frontiers = frontier_waypoints(boundaries_px)
        self.frontier_boundaries = [
            self._px_to_xy(np.asarray(boundary).reshape(-1, 2))
            for boundary in boundaries_px
        ]
        self.frontier_unexplored_directions = [
