...
def draw_robot(image, center, yaw, length, width, color=(255, 0, 0), thickness=2):
    """Draw a rectangular robot footprint with a heading arrow."""
    cx, cy = center
    c, s = np.cos(yaw), np.sin(yaw)
    R = np.array([[c, -s], [s, c]])

    half_L, half_W = length / 2, width / 2
    rect = np.array([[-half_L, -half_W], [half_L, -half_W], [half_L, half_W], [-half_L, half_W]])
    pts = (rect @ R.T + np.array([cx, cy])).astype(int)

    cv2.polylines(image, [pts], isClosed=True, color=color, thickness=thickness)
    front = np.mean(pts[1:3], axis=0).astype(int)
    cv2.arrowedLine(image, (int(cx), int(cy)), tuple(front), color, 2, tipLength=0.3)
