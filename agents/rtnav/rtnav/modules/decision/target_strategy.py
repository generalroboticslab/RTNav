...
class TargetStrategy:
    ...
    TARGET_POLICY_STOP_COMMIT_DIST_M = 0.75
    ...
    def __init__(self, shared_state, decision_cfg, camera_cfg):
        ...
        self._temporary_origin_xy = None
    ...
    def _start_target(
        ...
        self._temporary_origin_xy = None
        if target.get("temporary_target") and robot_xy is not None:
            origin = np.asarray(robot_xy[:2], dtype=float)
            if origin.shape == (2,) and np.isfinite(origin).all():
                self._temporary_origin_xy = (float(origin[0]), float(origin[1]))
    ...
    def finish_temporary_target(self, reached: bool, reason: str = "") -> None:
        ...
        with self.shared_state.lock:
            ...
            if reached and self._temporary_origin_xy is not None:
                origin = self._temporary_origin_xy
                if np.hypot(tx - origin[0], ty - origin[1]) > 1e-6:
                    self.shared_state.target.last_temporary_target_ray = (
                        origin,
                        (tx, ty),
                    )
        if reached:
        ...
        print(
            ...
            "continuing target/frontier selection"
        )
        self._current_target = None
        self._current_goal = None
        self._temporary_origin_xy = None
    ...
    def on_failed(self, reason: str):
        ...
        if reason in {"policy_stop", "no_progress"}:
            ...
            commit_dist = (
                self.TARGET_POLICY_STOP_COMMIT_DIST_M
                if reason == "policy_stop"
                else self.TARGET_COMMIT_DIST_M
            )
            within_commit_dist = dist is not None and (
                dist < commit_dist if reason == "policy_stop" else dist <= commit_dist
            )
            if not within_commit_dist:
                dist_txt = "unknown" if dist is None else f"{dist:.2f}m"
                comparison = ">=" if reason == "policy_stop" else ">"
                mode = (
                    "observe target"
                    if self._current_target.get("observe_mode")
                ...
                print(
                    ...
                    f"{dist_txt} away ({comparison} {commit_dist:.2f}m) — "
