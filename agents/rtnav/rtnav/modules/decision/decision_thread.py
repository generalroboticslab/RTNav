...
def should_refresh_frontier_goal(drift_m, threshold_m, old_goal_blocked):
    """Obstacle invalidation bypasses the normal anti-jitter drift threshold."""
    return bool(old_goal_blocked or drift_m >= threshold_m)
...
class DecisionThread:
    ...
    def _reset_for_new_episode_if_needed(self, snap: TickSnapshot):
        ...
        with self.shared_state.lock:
            ...
            self.shared_state.target.last_temporary_target_ray = None
    ...
    def _refresh_active_frontier(self, snap: TickSnapshot, nav: NavUpdate) -> bool:
        ...
        blocked = is_goal_blocked(snap.obstacle_map, old_xy)
        if not should_refresh_frontier_goal(
            drift, self.FRONTIER_GOAL_REFRESH_M, blocked
        ):
            return False
        print(
            f"[Decision] Updating {'blocked' if blocked else 'drifting'} frontier "
            f"({old_xy[0]:.1f}, "
    ...
    def _handle_no_frontier(self, reason: str):
        ...
        self._spin_wait_start = None
        self._pending_goal = None
        self.frontier.reset(keep_reached=True)
        with self.shared_state.lock:
