...
class FrontierStrategy:
    ...
    def __init__(self, shared_state, frontier_cfg, decision_cfg):
        ...
        self._reached_radius_m = float(frontier_cfg.strategy_reached_radius_m)
        ...
        self._last_chosen_idx: Optional[int] = None

        self._failures: list[FailedFrontier] = []
        self._reached_frontiers: list[Tuple[float, float]] = []
    ...
    def select_goal(
        ...
        unreached = [i for i, xy in enumerate(centroids_xy) if not self._was_reached(xy)]
        if not unreached:
            self._last_selection_time = now
            return None, "all frontiers reached"
        keep = [i for i in unreached if not self._is_banned(centroids_xy[i])]
        if not keep:
        ...
        temporary_target_ray = self._last_temporary_target_ray()
        if temporary_target_ray is not None:
            if not self._frontiers_are_fresh(frontier_output, now):
                self._last_selection_time = now
                return None, "waiting for fresh frontier detection"
            result = self._select_along_temporary_target_ray(
                clusters,
                centroids,
                centroids_xy,
                robot_xy,
                obstacle_map,
                temporary_target_ray,
            )
        ...
        self._sync_images(frontier_output, obstacle_map, centroids_xy, keep)
    ...
    def _last_temporary_target_ray(self):
        with self.shared_state.lock:
            ray = getattr(self.shared_state.target, "last_temporary_target_ray", None)
        if ray is None:
            return None
        origin, target = (np.asarray(point, dtype=float) for point in ray)
        if (
            origin.shape != (2,)
            or target.shape != (2,)
            or not np.isfinite(origin).all()
            or not np.isfinite(target).all()
            or np.linalg.norm(target - origin) <= 1e-6
        ):
            return None
        return origin, target

    def _select_along_temporary_target_ray(
        self,
        clusters,
        centroids,
        centroids_xy,
        ...
        origin_xy, target_xy = target_ray
        direction = target_xy - origin_xy
        direction /= np.linalg.norm(direction)
        choices = []
        for idx, (centroid_px, centroid_xy) in enumerate(zip(centroids, centroids_xy)):
            if self._was_reached(centroid_xy):
                continue
            nav = obstacle_map.find_navigable_frontier_goal(centroid_px)
            if nav is None or self._is_banned(nav[0]):
                continue
            delta = np.asarray(centroid_xy, dtype=float) - target_xy
            forward = max(0.0, float(delta @ direction))
            ray_distance = float(np.linalg.norm(delta - forward * direction))
            robot_distance = float(
                np.linalg.norm(np.asarray(centroid_xy, dtype=float) - robot_xy)
            )
            choices.append((ray_distance, robot_distance, idx, nav))
        if not choices:
            return None
        ray_distance, _robot_distance, idx, (nav_xy, nav_px) = min(choices)
        changed = idx != self._last_chosen_idx
        self._remember_choice(idx, nav_xy, centroids_xy, robot_xy)
        if changed:
            print(
                f"[Frontier] post-temporary ray bias -> F{idx} "
                f"({nav_xy[0]:.1f}, {nav_xy[1]:.1f}) "
                f"ray_distance={ray_distance:.1f}m"
            )
        return nav_xy, nav_px, clusters[idx]
    ...
    def _select_geometric(
        ...
        choices = []
        for idx, (cluster, centroid_px, centroid_xy) in enumerate(
    ...
    def refresh_goal(self, context: Dict[str, Any]):
        ...
        temporary_target_ray = self._last_temporary_target_ray()
        robot_pose = context.get("robot_pose")
        if temporary_target_ray is not None and robot_pose is not None:
            result = self._select_along_temporary_target_ray(
                clusters,
                centroids,
                centroids_xy,
                np.asarray(robot_pose[:2], dtype=float),
                obstacle_map,
                temporary_target_ray,
            )
            if result is not None:
    ...
    def on_goal_reached(self, goal_xy):
        ...
        reached_xy = self._last_frontier_xy if self._last_frontier_xy is not None else goal_xy
        self._reached_frontiers.append((float(reached_xy[0]), float(reached_xy[1])))
    ...
    def _was_reached(self, xy) -> bool:
        radius2 = self._reached_radius_m**2
        return any(
            (reached[0] - xy[0]) ** 2 + (reached[1] - xy[1]) ** 2 <= radius2
            for reached in self._reached_frontiers
        )
    ...
    def _sync_images(self, frontier_output, obstacle_map, centroids_xy, frontier_indices=None):
        cameras, recent_cameras = self._camera_sources()
        clusters = list(getattr(frontier_output, "frontier_clusters", []) or [])
        directions = list(
            getattr(frontier_output, "frontier_unexplored_directions", None) or []
        )
        if frontier_indices is not None:
            clusters = [clusters[i] for i in frontier_indices]
            directions = [directions[i] if i < len(directions) else None for i in frontier_indices]
        self.image_vlm.sync(
            centroids_xy,
            unexplored_directions_xy=directions,
            frontier_points_xy=[
                obstacle_map.px_to_xy(np.asarray(cluster).reshape(-1, 2))
    ...
    def clear_sticky(self, reason: str = ""):
        self._clear_sticky_state()
        self._last_state_action = None
        self._clear_viz_state()
        if reason:
            print(f"[Frontier] cleared sticky frontier: {reason}")

    def _clear_sticky_state(self):
        self._last_frontier_xy = None
        self._last_value = float("-inf")
        self._last_chosen_idx = None
    ...
    def reset(self, *, keep_reached: bool = False):
        self._clear_sticky_state()
        self._last_selection_time = 0.0
        self._need_fresh_after_ts = 0.0
        self._failures = []
        if not keep_reached:
            self._reached_frontiers = []
    ...
    def get_state(self):
        return {
            ...
            "reached_frontiers": len(self._reached_frontiers),
        }
