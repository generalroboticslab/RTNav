...
def ensure_vllm_server(
    ...
    max_num_seqs = int(os.environ.get("RTNAV_VLLM_MAX_NUM_SEQS", "1"))
    startup_timeout = float(
        os.environ.get("RTNAV_VLLM_STARTUP_TIMEOUT", str(max_wait))
    )
    ...
    if vllm_process_running():
        ...
        _time.sleep(0.5)
        print(
            f"[agent] starting vllm server (gpu_mem={gpu_mem}, port={port}, "
            f"max_num_seqs={max_num_seqs}, startup_timeout={startup_timeout:g}s) "
            f"for {model_dir} …"
        )
        print(
    ...
    start = _time.time()
    try:
        while _time.time() - start < startup_timeout:
    ...
    raise RuntimeError(
        f"[agent] vllm server did not become ready within {startup_timeout:g}s\n"
        f"--- last lines of {log_path} ---\n{_log_tail()}"
    )
