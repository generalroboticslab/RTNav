...
# --- rt_ovn author addition (begin): action-trace dump for the reproduction probe ---
def _gamap_trace_append(trace, infos):
    """Record worker-0 (action, step) into the probe trace. No-op when tracing off.

    Handles GAMap's own auto-reset ordering. When the agent picks STOP, three
    things happen inside one loop iteration in the same tick:

    1. ``multi_attr_exp.plan_act_and_preprocess`` runs habitat's step (with the
       STOP replaced by rotate=3), sets ``self.info['last_action']=0``, and
       returns ``done=True``.
    2. ``vector_env.PLAN_ACT_AND_PREPROCESS`` handler sees ``done`` and calls
       ``env.reset()``; that reset overwrites ``self.info['time']=0`` and
       ``self.info['sensor_pose']=[0,0,0]`` but leaves the just-written
       ``last_action=0`` in the dict. The info that reaches this appender is
       therefore ``(a=0, t=0)`` — with reset-time fields but a real STOP action.
       We keep this record; it's the only trace of the STOP.
    3. The next ga.py loop iteration handles ``done``, sets ``wait_env[0]=1``,
       and runs ``rotate_at_beginning_act`` in wait mode. That wait branch only
       clears ``self.last_action`` (attribute), not ``self.info['last_action']``,
       so the appender fires a second time with an identical ``(a=0, t=0)``.
       This is the ghost we drop. The ROS agent's ``while not done`` eval loop
       exits after step 2, so it never emits a ghost.

    A timeout takes a different path. ``get_done`` returns True from the
    ``time >= max_episode_length - 1`` branch rather than the ``self.stopped``
    branch, so there is no STOP and ``info['last_action']`` holds whatever the
    agent last did. The wait-mode reappend above then repeats *that* action
    instead of a 0, and the loop stays in wait mode for several iterations, so a
    timed-out episode picks up a run of ghosts rather than a single one.
    Observed at a 532 cap: nine trailing ``t=0`` records, ``2`` nine times on
    episodes 14/23 but ``1`` nine times on episode 24 — nine consecutive
    MOVE_FORWARDs are not a rotate-at-beginning, which is what confirms these
    are re-stamped stale values and not executed motion.

    Rule: skip any ``t=0`` record whose predecessor was also ``t=0``. Real steps
    are numbered from 1, so ``t=0`` only ever occurs inside the reset/wait
    window; keeping the first and dropping consecutive repeats preserves the
    STOP on a normal episode and collapses a timeout's ghost run to the single
    real post-cap record. A timed-out episode is then exactly
    ``max_episode_length`` actions long, which matches the ROS port's
    cap-exact behavior.
    """
    if trace is None or len(infos) == 0:
        return
    info = infos[0]
    a = info.get("last_action", None)
    if a is None:
        return
    a = int(a)
    t = int(info.get("time", 0) or 0)
    if t == 0 and trace["actions"]:
        prev = trace["steps"][-1]
        if int(prev.get("time", -1)) == 0:
            return  # consecutive ghost from wait-mode reappend (STOP or timeout)
    ...
    step = {
        "action": a,
        "time": int(info.get("time", -1)),
        "sensor_pose": list(info.get("sensor_pose", []) or []),
        "distance_to_goal": dtg,
    }
    trace["actions"].append(a)
    trace["steps"].append(step)

    # rt_ovn author addition: group into per-episode records so the comparator
    # can match episodes by identity instead of walking a cumulative offset
    # through one flat stream, where a single length difference shifts every
    # later episode. Only a t>=1 record may open a new episode: real steps are
    # numbered from 1, and the t=0 record is emitted after env.reset() has
    # already advanced info to the *next* episode, so it belongs to the episode
    # that just ended rather than the one whose id it now carries.
    if t >= 1:
        key = (str(info.get("scene_id", "")), str(info.get("episode_id", "")))
        if not trace["episodes"] or trace["episodes"][-1]["_key"] != key:
    ...
    if trace["episodes"]:
        trace["episodes"][-1]["actions"].append(a)
        trace["episodes"][-1]["steps"].append(step)
...
def main():
    ...
    # rt_ovn author addition: optional action-trace dump (GAMAP_TRACE_PATH) for the probe.
    trace_path = os.environ.get("GAMAP_TRACE_PATH", "").strip()
    trace = None
    if trace_path:
        os.makedirs(os.path.dirname(trace_path) or ".", exist_ok=True)
        trace = {"source": "official_gamap", "episode": {}, "first_observation": {},
                 "actions": [], "steps": [], "episodes": [], "final": {}}
    ...
    # rt_ovn author addition: capture episode metadata for the probe trace.
    if trace is not None and len(infos) > 0:
        _i0 = infos[0]
        trace["episode"] = {
            "scene_id": str(_i0.get("scene_id", "")),
            "episode_id": str(_i0.get("episode_id", "")),
            "goal_name": _i0.get("goal_name", ""),
        }
    ...
    _gamap_trace_append(trace, infos)  # rt_ovn author addition
    ...
    # rt_ovn author addition: write the probe action trace (GAMAP_TRACE_PATH).
    if trace is not None:
        _fi = infos[0] if len(infos) > 0 else {}
        _dtg = _fi.get("distance_to_goal", None)
        try:
            _dtg = float(_dtg)
        except (TypeError, ValueError):
            _dtg = None
        trace["final"] = {
            "success": float(_fi.get("success", 0.0) or 0.0),
            "spl": float(_fi.get("spl", 0.0) or 0.0),
            "distance_to_goal": _dtg,
            "num_actions": len(trace["actions"]),
        }
        for _ep in trace["episodes"]:
            _ep.pop("_key", None)  # scratch grouping key, not part of the schema
        with open(trace_path, "w") as f:
            json.dump(trace, f, indent=2)
        print("Saved GAMap trace to {} ({} actions, {} episodes)".format(
            trace_path, len(trace["actions"]), len(trace["episodes"])))
