...
def make_vec_envs(args):
    # rt_ovn author addition: lazy habitat import (matches L3MVN) so the ROS agent
    # can import envs.utils.* (via model_multi) without the habitat package, which
    # lives only in the env container. No behavioral change: ga.py still calls
    # make_vec_envs, which imports habitat here exactly as before.
