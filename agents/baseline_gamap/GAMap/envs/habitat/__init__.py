...
def make_env_fn(args, config_env, rank):
    ...
    _apply_data_dir_to_dataset(dataset)  # rt_ovn author addition
    config_env.defrost()
...
# --- rt_ovn author addition (begin): DATA_DIR-based path resolution ----------
# Lets the official run read datasets/scenes from $DATA_DIR (docs/datasets.md)
# instead of a repo-local data/ folder of symlinks. Both helpers are no-ops when
# DATA_DIR is unset, so upstream behaviour is unchanged.
def _apply_data_dir(cfg):
    """When DATA_DIR is set, rewrite the repo-relative ``data/`` paths to the
    shared dataset root so the official run needs no ``data/`` symlinks. Paths
    match the layout used by the rt_ovn probe / VLFM (``$DATA_DIR`` =
    dataset root from docs/datasets.md). No-op when DATA_DIR is unset, so the
    upstream behaviour (relative ``data/`` folder) is preserved."""
    data_dir = os.environ.get("DATA_DIR", "").strip()
    if not data_dir:
        return
    cfg.DATASET.DATA_PATH = os.path.join(
        data_dir, "datasets/objectnav/hm3d/v1/{split}/{split}.json.gz")
    cfg.DATASET.SCENES_DIR = os.path.join(data_dir, "versioned_data/hm3d-0.2")
    cfg.SIMULATOR.SCENE_DATASET = os.path.join(
        data_dir,
        "versioned_data/hm3d-0.2/hm3d/hm3d_annotated_basis.scene_dataset_config.json")


def _apply_data_dir_to_dataset(dataset):
    """habitat.Env overwrites SIMULATOR.SCENE_DATASET from each episode's
    ``scene_dataset_config`` field on reset (habitat/core/env.py), and that
    field is baked into the episode files as a relative ``./data`` path. Rewrite
    it to the DATA_DIR-based absolute config so semantics load without a data/
    symlink. ``scene_id`` is already made absolute by make_dataset via
    SCENES_DIR, so only this field needs fixing. No-op when DATA_DIR is unset."""
    data_dir = os.environ.get("DATA_DIR", "").strip()
    if not data_dir:
        return
    ...
    for ep in dataset.episodes:
        ep.scene_dataset_config = cfg_path
...
def construct_envs21(args):
    ...
    # basic_config.DATASET.EPISODES_DIR = \
    #     basic_config.DATASET.EPISODES_DIR.replace("v1", args.version)
    _apply_data_dir(basic_config)  # rt_ovn author addition
    basic_config.freeze()
    ...
    for i in range(args.num_processes):
        ...
        # config_env.DATASET.EPISODES_DIR = \
        #     config_env.DATASET.EPISODES_DIR.replace("v1", args.version)
        _apply_data_dir(config_env)  # rt_ovn author addition
