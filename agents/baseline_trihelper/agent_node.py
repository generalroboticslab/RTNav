#!/usr/bin/env python3
"""TriHelper ROS2 agent node for the shared env-agent evaluation pipeline.

The upstream TriHelper implementation owns both Habitat stepping and policy logic in
...
TRIHELPER_ROOT = "/opt/trihelper"
QWEN_MODEL = "pretrained_models/Qwen-VL"
DORMANT_THRESHOLD_CM = 50
SLEEP_TIME = 20
DETECTION_THRESHOLD = 400
HARD_GOAL_TIME = 30
if TRIHELPER_ROOT in sys.path:
    sys.path.remove(TRIHELPER_ROOT)
sys.path.insert(0, TRIHELPER_ROOT)
...
_COCO_OBJECTGOAL_TO_TRIHELPER = [0, 3, 2, 4, 5, 1]
_GOAL_ALIASES = {
    "chair": "chair",
    "bed": "bed",
    "plant": "plant",
...
class UnTrapHelper:
    """Upstream TriHelper's alternating two-turn blocked-agent recovery."""
    ...
    def get_action(self):
        self.epi_id += 1
        if self.epi_id == 1:
            return LEFT if self.total_id % 2 == 0 else RIGHT
        return RIGHT if self.total_id % 2 == 0 else LEFT


class TriHelperLocalPlanner:
    ...
    def __init__(self, args, device, rednet_path):
        ...
        self.timestep = 0
        self.episode_no = 0
        self._reset_flag_after_preprocess = False
    ...
    def _plan(self, planner_inputs):
        ...
        if self.prev_blocked >= self.block_threshold and pre_eve_angle == 0:
            self.eve_angle = 0
            if self._previous_action == FORWARD:
                action = self.untrap.get_action()
                decision_branch = "untrap_turn"
            else:
                action = FORWARD
                decision_branch = "untrap_forward"
    ...
    def _preprocess_obs(self, obs, use_seg=True):
        ...
        sem_seg_pred = self._verify_semantics_with_qwen(rgb, sem_seg_pred)
        if self._reset_flag_after_preprocess:
            self.flag = 0
            self._reset_flag_after_preprocess = False
    ...
    def _verify_semantics_with_qwen(self, rgb, sem_seg_pred):
        goal_cat_id = int(self.info["goal_cat_id"])
        goal_name = self.info["goal_name"]
        if (
            self.timestep >= DETECTION_THRESHOLD
            or self.flag != 0
            or not np.any(sem_seg_pred[:, :, goal_cat_id] == 1)
        ):
            return sem_seg_pred
        ...
        image_dir = os.path.join(self.args.dump_location, "path")
        os.makedirs(image_dir, exist_ok=True)
        image_path = os.path.join(
            image_dir,
            f"ask{goal_name}-{self.episode_no}-{self.timestep}-{self.rank}.png",
        )
        cv2.imwrite(image_path, rgb[:, :, ::-1])
        query = self.qwen_tokenizer.from_list_format(
            [
                {"image": image_path},
                {"text": f"这张图中有{target_zh}吗"},
            ]
        )
        response, self.history = self.qwen_model.chat(
            self.qwen_tokenizer,
            query=query,
        ...
        accepted = None
        if any(token in response_text for token in ("0", "没", "不")):
            sem_seg_pred[:, :, goal_cat_id] = 0
            self.jiamubiao = 1
            self.zhaodaoguole = 1
            accepted = False
        elif any(token in response_text for token in ("1", "存在", "有")):
            self.flag = 1
            self.jiamubiao = 0
            accepted = True
...
class TriHelperPolicy:
    """Single-environment TriHelper zero-shot policy state machine."""
    ...
    def __init__(self, node: BaseAgentNode):
        ...
        self.trace_path = os.environ.get("AGENT_TRACE_PATH", "").strip()
        self.trace = None
        self.trace_episodes = []
        self._last_state_debug = None
        self.set_llm_xiumian = 0
        self.set_hard_goal = 0
        self.xiumiancount = 0
        self.yougoal = 0
        self.pregoal = None
        self.current_goal = None
        self._last_goal_map = np.zeros((self.local_w, self.local_h), dtype=np.float32)
        self.episode_no = 0
    ...
    def _make_args(self):
        return SimpleNamespace(
            ...
            split="val",
            version="v1",
            dump_location="/opt/worker_logs/trihelper_tmp",
            exp_name="ros_trihelper_zero",
            visualize=0,
            print_images=0,
            env_frame_width=640,
            ...
            use_gtsem=0,
            sem_gpu_id=0 if torch.cuda.is_available() else -2,
        )

    def _make_planner_agent(self):
        return TriHelperLocalPlanner(
            self.args,
            self.device,
            "data/rednet_semmap_mp3d_40.pth",
        )
    ...
    def reset(self, obs_dict: dict, target_name: Optional[str] = None):
        ...
        p.prev_blocked = 0
        p.block_threshold = 6
        p._previous_action = -1
        p.history = None
        p.jiamubiao = 0
        p.zhaodaoguole = 0
        p.episode_no = self.episode_no
        p.timestep = 0
        p._reset_flag_after_preprocess = True
        p._last_vlm_debug = None
    ...
    def act(self, obs_dict: dict) -> Tuple[int, dict]:
        ...
        self.planner_agent.info = info
        self.planner_agent.timestep = self.decision_step
    ...
    def _first_action(self, obs_dict: dict) -> int:
        ...
        self._update_goal_rest_state(planner_input, info)
    ...
    def _append_trace(self, action: int, info: dict, sim_location, main_step: int, l_step: int):
        ...
        rec = {
            ...
            "set_llm_xiumian": int(info.get("set_llm_xiumian", 0)),
            "set_hard_goal": int(info.get("set_hard_goal", 0)),
            ...
            "vlm_debug": getattr(self.planner_agent, "_last_vlm_debug", None),
    ...
    def _main_loop_action(self, obs: torch.Tensor, info: dict) -> int:
        ...
        for e in range(self.num_scenes):
            ...
            if (
                np.any(self.target_point_map[e] == global_item + 1)
                and not self.set_llm_xiumian
            ):
            ...
            if self.local_map[e, cn, :, :].sum() != 0.0 and not self.set_hard_goal:
            ...
            if self.set_hard_goal:
                found_goal[e] = 0
                local_goal_maps[e].fill(0.0)
                goal_r, goal_c = self._collision_helper_goal(e)
                local_goal_maps[e][goal_r, goal_c] = 1
                goal_debug[e]["goal_source"] = "collision_helper"
        ...
        self._last_goal_map = np.asarray(local_goal_maps[0], dtype=np.float32).copy()
    ...
    def _collision_helper_goal(self, e: int):
        explored = self.local_map[e, 1, :, :].cpu().numpy() > 0.5
        obstacle = self.local_map[e, 0, :, :].cpu().numpy() > 0.5
        navigable = np.logical_and(explored, np.logical_not(obstacle))
        labels = measure.label(navigable, connectivity=2)
        regions = measure.regionprops(labels)
        if not regions:
            return self.local_w // 2, self.local_h // 2

        largest = max(regions, key=lambda region: region.area)
        center = np.asarray(largest.centroid)
        region_cells = largest.coords
        nearest = region_cells[np.argmin(np.sum((region_cells - center) ** 2, axis=1))]
        return int(nearest[0]), int(nearest[1])

    def _update_goal_rest_state(self, planner_input: dict, info: dict):
        """Mirror TriHelper's 20-step pause after revisiting a nearby frontier."""
        if planner_input["new_goal"]:
            self.yougoal = 1
            self.current_goal = planner_input["goal"]

        if self.yougoal and not self.set_hard_goal:
            goal_cells = np.argwhere(self.current_goal > 0.5)
            if goal_cells.size:
                goal_r, goal_c = goal_cells[0]
                agent_r = float(self.local_pose[0, 1]) * 100 / self.args.map_resolution
                agent_c = float(self.local_pose[0, 0]) * 100 / self.args.map_resolution
                dist_cm = (
                    pu.get_l2_distance(goal_r, agent_r, goal_c, agent_c)
                    * self.args.map_resolution
                )
                if (
                    dist_cm < DORMANT_THRESHOLD_CM
                    and self.planner_agent.replan_count == 0
                    and planner_input["new_goal"]
                    and self.pregoal is not None
                    and np.array_equal(self.pregoal, self.current_goal)
                ):
                    self.set_llm_xiumian = 1
            self.yougoal = 0

        if self.set_llm_xiumian:
            self.xiumiancount += 1
            if self.xiumiancount == SLEEP_TIME:
                self.set_llm_xiumian = 0
                self.xiumiancount = 0

        if self.set_hard_goal:
            self.xiumiancount += 1
            if self.xiumiancount == HARD_GOAL_TIME:
                self.set_hard_goal = 0
                self.xiumiancount = 0

        if planner_input["new_goal"]:
            self.pregoal = np.copy(planner_input["goal"])

        info["set_llm_xiumian"] = int(self.set_llm_xiumian)
        info["set_hard_goal"] = int(self.set_hard_goal)
    ...
    def _info(self, sensor_pose):
        return {
            ...
            "set_llm_xiumian": int(self.set_llm_xiumian),
            "set_hard_goal": int(self.set_hard_goal),
        }

    def _update_pending_clear_flag(self, new_goal=False):
        clear_flag = int(
            getattr(self.planner_agent, "collision_n", 0) > 20
            or getattr(self.planner_agent, "replan_count", 0) > 50
        )
        if clear_flag:
    ...
    def _resolve_goal(self, obs_dict: dict, target_name: Optional[str]):
        if "objectgoal" in obs_dict:
            ...
            if 0 <= objectgoal < len(category_to_id):
                return _COCO_OBJECTGOAL_TO_TRIHELPER[objectgoal], category_to_id[objectgoal]
        if target_name:
            alias = _GOAL_ALIASES.get(target_name.strip().lower())
            if alias in category_to_id:
                return _COCO_OBJECTGOAL_TO_TRIHELPER[category_to_id.index(alias)], alias
        self.node.get_logger().warning(
            f"TriHelper target {target_name!r} is outside the original six-category vocabulary; defaulting to chair."
        )
        return 0, "chair"
            ...
            info.update(
                {
                    ...
                    "map_title": "TriHelper Semantic Map",
                }
            )
        return info

    def _semantic_video_map(self) -> np.ndarray:
        """Render the SemExp local map without changing planner state."""
        local = self.local_map[0].detach().cpu().numpy()
        semantic_scores = local[4:].copy()
        # TriHelper reserves the final semantic channel for cells with no
        # detected category. The upstream visualizer gives that channel a
        # tiny value before argmax so empty cells do not appear as chairs.
        semantic_scores[-1] = np.maximum(semantic_scores[-1], 1e-5)
        labels = semantic_scores.argmax(axis=0).astype(np.uint8) + 5

        no_category = labels == 20
...
class TriHelperAgentNode(BaseAgentNode):
    """TriHelper zero-shot ObjectNav agent."""

    def __init__(self, mode: str = "sync"):
        super().__init__("trihelper_agent_node", mode)
        self._policy: Optional[TriHelperPolicy] = None
        self._episode_targets = {}
        self.create_subscription(String, "/episode_info_json", self._on_episode_info_json, 10)
        os.chdir(TRIHELPER_ROOT)

    def _load_policy(self):
        self._policy = TriHelperPolicy(self)
        self.get_logger().info("TriHelper zero-shot policy loaded.")
    ...
    def _on_episode_start(self, reset_resp, initial_obs: dict):
        ...
        self.get_logger().info(f"TriHelper target: {self._policy.goal_name}")

    def _compute_action(self, obs_dict: dict, step: int) -> Tuple[int, dict]:
        # TriHelper keeps map tensors as mutable episode state. torch.inference_mode()
