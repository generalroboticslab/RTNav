...
class FailureModeMeasure(Measure):
    ...
    def _shifted_object_name(self, object_name, shift):
        if not isinstance(object_name, str) or "_" not in object_name:
            return None
        name, suffix = object_name.rsplit("_", 1)
        if not suffix.isdigit():
            return None
        shifted_suffix = int(suffix) + shift
        if shifted_suffix < 0:
            return None
        return f"{name}_{shifted_suffix}"

    def _goal_object_candidates(self, goal):
        candidates = []
        ...
        if isinstance(object_name, str):
            ...
            shifted_name = self._shifted_object_name(object_name, -2)
            if shifted_name is not None:
                candidates.append(shifted_name)
        return candidates

    def _resolve_goal_objects(self, goals, semantic_scene, episode):
        semantic_objects = [o for o in semantic_scene.objects if o is not None]
        objects_by_id = {str(o.id): o for o in semantic_objects}
        objects_by_semantic_id = {
            str(o.semantic_id): o
            for o in semantic_objects
            if getattr(o, "semantic_id", None) is not None
        }
        ...
        for goal in goals:
            object_id = getattr(goal, "object_id", None)
            object_name = getattr(goal, "object_name", None)
            candidates = self._goal_object_candidates(goal)

            match = None
            for candidate in candidates:
                semantic_obj = objects_by_id.get(candidate)
                if semantic_obj is None:
                    # OVON goal object_id values come from the numeric IDs in
                    # HM3D's semantic annotation file. Habitat exposes those
                    # values as semantic_id; object.id uses a scene-qualified
                    # identifier in some Habitat-Sim versions.
                    semantic_obj = objects_by_semantic_id.get(candidate)
        ...
        if missing:
            scene_id = episode.scene_id.split("/")[-1]
            raise RuntimeError(
                "[FailureModeMeasure] FATAL: failed to map goal objects to "
                "Habitat semantic objects; "
                f"scene={scene_id} category={episode.object_category} "
                f"missing={missing}"
            )

        unique = {}
        for obj in resolved:
            unique[str(obj.id)] = obj
    ...
    def visible_goal_area(self, observations, episode, task):
        ...
        objs = self._resolve_goal_objects(goals, semantic_scene, episode)
