...
class HabitatEnvWrapper:
    ...
    """
    STOP, FORWARD, LEFT, RIGHT, LOOK_UP, LOOK_DOWN, SUBTASK_STOP = 0, 1, 2, 3, 4, 5, 6
    ...
    @property
    def active_subtask_index(self) -> int:
        task = getattr(self._habitat_env, "task", None)
        return int(getattr(task, "active_subtask_idx", 0))
...
class AsyncHabitatEnv(HabitatEnvWrapper):
    ...
    ``habitat_env`` / ``current_episode`` / ``number_of_episodes`` /
    ``active_subtask_index`` / ``close`` surface. Overrides ``velocity_step``
