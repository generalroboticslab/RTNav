...
class EnvNode(Node):
    ...
    def _publish_episode_info(self, episode, payload=None):
        ...
        self._publish_if_context_valid(self._pub_ep, ep_msg)
        ...
        self._publish_if_context_valid(self._pub_ep_json, ep_json)

    def _publish_if_context_valid(self, publisher, msg) -> bool:
        """Publish unless ROS shutdown has already invalidated this context.

        The default rclpy SIGINT/SIGTERM handler can invalidate the context
        while a timer callback is still running.  Treat only that shutdown
        race as a clean stop; publish failures with a live context remain
        genuine errors and are re-raised.
        """
    ...
    def shutdown(self):
        sim_timer = getattr(self, "_sim_timer", None)
...
class AsyncEnvNode(EnvNode):
    ...
    def __init__(self, config: DictConfig, benchmark: str = "ovon", video_dir: str = ""):
        ...
        self._sim_hz_target = float(hz)
        self._sim_timer = self.create_timer(
            self._sim_dt, self._async_tick, callback_group=self._cb,
        )

        # Sim-rate diagnostic: log every 2s with measured Hz and per-tick stats.
        self._rate_log_period_s = float(os.environ.get("SIM_RATE_LOG_PERIOD_S", "2.0"))
        self._rate_window_t0 = time.time()
        self._rate_window_steps = 0
        self._rate_window_tick_ms_sum = 0.0
        self._rate_window_tick_ms_max = 0.0
        self._last_tick_end_t = None

        self.get_logger().info(
            f"Async env node ready (sim_hz={hz}, timeout={self._async_timeout}s, "
            f"rate_log_period={self._rate_log_period_s:.1f}s)."
        )
    ...
    def _async_tick(self):
        ...
        _tick_start_t = time.time()

        if _tick_start_t - self._ep_start_time > self._async_timeout:
        ...
        if not self._publish_if_context_valid(
            self._pub_obs,
            build_obs_msg(pack_obs_fields(obs, self._step_id, self._ep_hash)),
        ):
            return

        _tick_ms = (time.time() - _tick_start_t) * 1000.0
        self._rate_window_steps += 1
        self._rate_window_tick_ms_sum += _tick_ms
        if _tick_ms > self._rate_window_tick_ms_max:
            self._rate_window_tick_ms_max = _tick_ms
        self._last_tick_end_t = time.time()

        _window_elapsed = self._last_tick_end_t - self._rate_window_t0
        if _window_elapsed >= self._rate_log_period_s:
            measured_hz = self._rate_window_steps / _window_elapsed
            avg_ms = self._rate_window_tick_ms_sum / max(1, self._rate_window_steps)
            max_ms = self._rate_window_tick_ms_max
            target_ms = 1000.0 * self._sim_dt
            lag = "" if measured_hz >= 0.95 * self._sim_hz_target else "  ← LAG"
            # self.get_logger().info(
            #     f"[sim-rate] {measured_hz:5.1f} Hz over {_window_elapsed:.1f}s "
            #     f"({self._rate_window_steps} steps) | tick avg={avg_ms:.1f}ms "
            #     f"max={max_ms:.1f}ms target={target_ms:.1f}ms{lag}"
            # )
            self._rate_window_t0 = self._last_tick_end_t
            self._rate_window_steps = 0
            self._rate_window_tick_ms_sum = 0.0
            self._rate_window_tick_ms_max = 0.0
    ...
    def _pub_done_sentinel(self, obs):
        self._publish_if_context_valid(
            self._pub_obs,
            build_obs_msg(pack_obs_fields(obs, DONE_SENTINEL, self._ep_hash)),
        )
